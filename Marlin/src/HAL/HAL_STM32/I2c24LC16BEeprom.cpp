...
void eeprom_write_byte(unsigned char *pos, unsigned char value) {
	...
	Wire.beginTransmission(makeDevAddress(eeprom_address));
	Wire.write(get_LowAddress(eeprom_address)); // LSB
  Wire.write(value);
...
unsigned char eeprom_read_byte(unsigned char *pos) {
  ...
  Wire.write(get_LowAddress(eeprom_address)); // LSB
	Wire.endTransmission();
	Wire.requestFrom(makeDevAddress(eeprom_address), (byte)1);
	if (Wire.available())
		data = Wire.read();
	return data;
}
