...
#ifndef	_FASTIO_STM32_H
#define	_FASTIO_STM32_H
...
/// Read a pin
#define _READ(IO) digitalRead(IO)//((bool)(DIO ## IO ## _WPORT -> PIO_PDSR & (MASK(DIO ## IO ## _PIN))))

/// Write to a pin
#define _WRITE_VAR(IO, v) digitalWrite(IO,v)//do {  if (v) {g_APinDescription[IO].pPort->PIO_SODR = g_APinDescription[IO].ulPin; } \
                          //          else {g_APinDescription[IO].pPort->PIO_CODR = g_APinDescription[IO].ulPin; } \
                          //       } while (0)

#define _WRITE(IO, v) digitalWrite(IO,v)//do {  if (v) {DIO ## IO ## _WPORT -> PIO_SODR = MASK(DIO ## IO ##_PIN); } \
                      //          else {DIO ##  IO ## _WPORT -> PIO_CODR = MASK(DIO ## IO ## _PIN); }; \
                      //       } while (0)

/// toggle a pin
#define _TOGGLE(IO)  digitalWrite(IO,!digitalRead(IO))//_WRITE(IO, !READ(IO))

/// set pin as input
#define _SET_INPUT(IO)  pinMode(IO,INPUT)//pmc_enable_periph_clk(g_APinDescription[IO].ulPeripheralId); \
                        //PIO_Configure(g_APinDescription[IO].pPort, PIO_INPUT, g_APinDescription[IO].ulPin, 0)
/// set pin as output
#define _SET_OUTPUT(IO)  pinMode(IO,OUTPUT)//PIO_Configure(g_APinDescription[IO].pPort, PIO_OUTPUT_1, \
